...
interface DemoTarget extends ServiceRequestTarget {
...
const sourceDataset = "SR2026_only_new_in_progress_potholes.csv";
const minimumEventSpacingMs = 18 * 60 * 1000;
...
const demoTargets: DemoTarget[] = serviceRequestTargets.map((target, index) => ({
  ...target,
  status: "monitoring",
  sourceCount: sourceCountFor(target, index),
  eventsPerSource: eventsPerSourceFor(target, index),
  baseImpactMagnitude: averageImpactMagnitudeFor(target, index),
}));
...
export function buildDemoSeedEvents(seed: string): Array<{
  ...
  for (const target of demoTargets) {
    const eventCount = target.sourceCount * target.eventsPerSource;
    const eventSpacingMs = spacingMsFor(target, eventCount);
    const firstReportedAt = new Date(target.firstReportedAt).getTime();
      ...
      for (let eventIndex = 0; eventIndex < target.eventsPerSource; eventIndex += 1) {
        ...
        events.push({
          ...
          event: {
            ...
            speed: 24 + (offset % 6) * 5,
            heading: (230 + offset * 11) % 360,
            impactMagnitude: round1(target.baseImpactMagnitude + (eventIndex % 2) * 0.4),
            verticalAcceleration: round1(target.baseImpactMagnitude / 3),
            gpsAccuracyMeters: 5 + (offset % 4),
            timestamp: new Date(firstReportedAt + offset * eventSpacingMs).toISOString(),
            sourceType: "seeded_demo",
            appVersion: "demo-seed-sr2026",
            sensorWindowSummary: {
              samples: 16 + (offset % 6),
              target: target.address,
              ward: target.ward,
              originalStatus: "In Progress",
              serviceRequestType: target.serviceRequestType,
...
function spacingMsFor(target: DemoTarget, eventCount: number) {
  const firstReportedAt = new Date(target.firstReportedAt).getTime();
  const lastReportedAt = new Date(target.lastReportedAt).getTime();

  if (eventCount <= 1) {
    return 0;
  }

  return Math.max(minimumEventSpacingMs, Math.floor((lastReportedAt - firstReportedAt) / (eventCount - 1)));
}
